...
- **`review-run`** (this) → how a multi-agent **run** went → a report in `.claude/retros/`, plus
...
(`subagent_tokens`, `tool_uses`, `duration_ms`) and a status (`completed` / `failed` / `killed`).
Read those from the conversation you just conducted:
1. **Per-agent telemetry** — from each task-notification's `<usage>` + status.
...
3. **Optional exact telemetry (fallback)** — if the notifications have scrolled out of context, a
   small script MAY parse the per-task transcript files (`…/tasks/<id>.output`, JSONL) for the
   trailing usage record ONLY — extract aggregate fields (tokens/tool-uses/duration/status) and
   **never emit transcript bodies into context** (they overflow it).

**Honesty rule:** if telemetry is partial (context was summarized, a script couldn't run), say so
and mark the number `unknown` — never invent a token or agent count. A retro with three real rows
and two "unknown" rows is worth more than five guessed ones.
...
- [ ] 3. METRICS — derive: total agents (and wasted/retried), total subagent tokens (and by phase / by
         model tier), wall-clock vs sum-of-agent-time (parallelism efficiency), fix-loop rounds (rework),
         failure/retry count. Mark any unknowns.
...
- [ ] 6. WRITE + ROUTE — save the report to `.claude/retros/RETRO-YYYY-MM-DD-<workflow>.md`; compare to the
         previous retro there for a trend if one exists; route ONLY durable, recurring orchestration learnings
         to project memory (link related notes, don't duplicate what the repo/git already records).
...
Scope: <phases covered> · Source: <in-context notifications | + tasks/*.output script | partial>
...
- Tokens: total (by phase; by model tier) · Wall-clock ≈ vs sum-of-agent-time (parallelism factor)
...
## Trend (vs prior retro, if any)
- Agents / tokens / rework: <up|down vs last run>
```
...
3. **Route, don't dump** — the per-run report lives in `.claude/retros/`; only durable, *recurring*
