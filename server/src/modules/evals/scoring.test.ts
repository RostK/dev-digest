...
describe('matchExpected', () => {
  it('matches same file + overlapping line range', () => {
    ...
    expect(matchExpected(expected, produced)).toBe(true);
  });
  ...
  it('does not match a different file even with overlapping lines', () => {
    ...
    expect(matchExpected(expected, produced)).toBe(false);
  });
  ...
  it('matches an inverted produced range (end_line < start_line) that overlaps', () => {
    // The Finding contract permits end_line < start_line; the whole [15, 20]
    // span must still be considered, not collapsed to just {20}.
    const expected = { file: 'src/a.ts', start_line: 16, end_line: 16 };
    const produced = makeFinding({ file: 'src/a.ts', start_line: 20, end_line: 15 });
    expect(matchExpected(expected, produced)).toBe(true);
  });
...
describe('scoreCase', () => {
  it('recall 0.5 — a must_find case with 2 expected + 1 matched produced finding', () => {
    ...
    const result = scoreCase({ expectation, produced, dropped: 0 });
