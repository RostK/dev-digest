...
| 2026-07-10 | write-spec + plan (SPEC-08 CI-export idempotent) | 3 / 0 | 243,320 | ? | 54 | 0 | 1.0 | zero-waste; firm total (no nesting — planner spawned 0 researchers); plan phase 113k, below the ~150k baseline (small fix + pre-grounded brief); only inefficiency was a *product-model* assumption (one-vs-many agents/repo) baked in the brief → re-litigated at the approval gate, not an NC | [↗](RETRO-2026-07-10-write-spec-plan-spec08.md) |
| 2026-07-10 | implement (SPEC-08 CI-export idempotent) | 4 / 0 | 390,657 | ? | 102 | 0 | 1.12× | cheapest+zero-waste implement to date; firm total (no nesting); single-agent build + cost-scaled /code-review (1 finder not 8) + 0 fix rounds; implementer 170k was the outlier (worktree setup: deps + agent-runner/dist rebuild dominated a 4-file change); real end-to-end verify achieved (7/7 .it real PG + real export self-healed PR #22 CI to green); only friction = Docker down mid-run (env precondition unchecked before a Docker-dependent verify phase) | [↗](RETRO-2026-07-10-implement-spec08.md) |
| 2026-07-11 | write-spec (SPEC-09 Agent Performance + SPEC-10 Plugin Export/Import) | 4 / 0 | 275,029 | ? | 117 | 0 | 1.69× | dual-spec parallel fan-out (2 author + 2 resolve pairs); firm total (no nesting); ~137k/spec — below single-spec baselines; 14 NCs → 1 batched ASK → 1 resolve pass, no re-loop; only defect = concurrent INDEX.md write race (both authors claimed SPEC-09; Plugin author self-renumbered to SPEC-10 + cross-linked) → fix: pre-assign spec IDs in parallel author briefs | [↗](RETRO-2026-07-11-write-spec.md) |
