...
## Engineering insights (session loop)

Per-module `INSIGHTS.md` files accumulate durable, non-obvious learnings so knowledge
compounds across sessions. Files: `client/`, `server/`, `reviewer-core/`,
`server/src/modules/repo-intel/`.

- **Before any work (mandatory):** after the user's prompt, read the relevant doc from
  `## Read when` above AND the touched/discussed module's `INSIGHTS.md`. Treat it as
  high-confidence guidance unless told otherwise.
- **End of a substantive session** (>30 min with a real problem/decision/discovery):
  run `/engineering-insights`. Append only learnings that are substantial AND not
  already in the file (re-read first; never duplicate). Nothing substantial → write
  nothing. Trivial edits → skip.
